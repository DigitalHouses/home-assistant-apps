...
def test_054_version_and_repository_validator_contract():
    assert (APP / "VERSION").read_text(encoding="utf-8").strip() == "0.5.4"
...
def test_054_operational_guide_contract():
...
def test_054_readme_and_changelog_document_installed_root_guide():
    readme = (APP / "README.md").read_text(encoding="utf-8")
    changelog = (APP / "CHANGELOG.md").read_text(encoding="utf-8")

    assert "`VERSION` is `0.5.4`." in readme
