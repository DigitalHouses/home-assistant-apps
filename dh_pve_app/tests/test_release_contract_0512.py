...
def test_0512_version_and_repository_validator_contract():
    assert (APP / "VERSION").read_text(encoding="utf-8").strip() == "0.5.12"

    text = VALIDATOR.read_text(encoding="utf-8")
    for required in (
        'EXPECTED_VERSION = "0.5.12"',
        'app / "dh_app_pve.txt"',
...
def test_0512_operational_guide_contract():
...
def test_0512_readme_and_changelog_document_installed_root_guide():
    readme = (APP / "README.md").read_text(encoding="utf-8")
    changelog = (APP / "CHANGELOG.md").read_text(encoding="utf-8")

    assert "`VERSION` is `0.5.12`." in readme
    assert "/root/dh_app_pve.txt" in readme

    assert "## 0.5.9" in changelog
    assert "## 0.5.12" in changelog
    assert "## 0.5.9" in changelog
