...
def _released_build(tmp_path: Path, version: str) -> Path:
    ...
    path.write_text(
        f"version = {version}\n"
        f"source = digitalhouses_plex_agent-v{version}\n"
...
def test_payload_is_exact_protocol_v1_without_site_or_plex_data(tmp_path: Path):
...
def test_enabled_released_build_sends_once_and_restart_does_not_storm(tmp_path: Path):
    now = {"value": 1000.0}
    store = StateStore(tmp_path / "telemetry.json")
    build = _released_build(tmp_path, "0.7.0")
...
def test_version_change_allows_one_immediate_best_effort_report(tmp_path: Path):
    ...
    build = _released_build(tmp_path, "0.7.1")
    transport = FakeTransport()
    ...
    assert transport.calls[0]["payload"]["version"] == "0.7.1"
    assert upgraded.installation_id == old.installation_id
    assert upgraded.installation_token == old.installation_token
