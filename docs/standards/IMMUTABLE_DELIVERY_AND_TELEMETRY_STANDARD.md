...
Default per-installation retention:

```text
TELEMETRY_RETENTION_DAYS=60
```

Expired installation-level records are deleted.
...
repeated heartbeat -> first_seen unchanged
version update -> version field updated
...
delete with valid token -> record removed
delete with ID only/wrong token -> rejected
retention -> expired record removed
```
...
Telemetry service:
- stores minimum required installation data
- derives only country
- does not retain source IP in the telemetry system
- supports activity/version/country views
- enforces retention
- supports authenticated deletion
