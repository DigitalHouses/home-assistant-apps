...
class TelemetryClient:
    ...
    def _load_or_create_state(self) -> dict[str, Any]:
        ...
        try:
            ...
            valid_id = parsed.version == 4 and str(parsed) == str(installation_id).lower()
        ...
        try:
            token_valid = isinstance(token, str) and len(bytes.fromhex(token)) >= 32
        except ValueError:
            token_valid = False
        if not token_valid:
